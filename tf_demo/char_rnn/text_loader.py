...
class TextLoader(): 
    ...
    def preprocess(self, input_file, vocab_file, tensor_file): 
        with codecs.open(input_file, "r", encoding=self.encoding) as f: 
            data = f.read() # 统计出每个字符出现多少次，统计结果总共有65个字符，所以vocab_size = 65
            r='[’!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~]+'
            ...
            self.vocab_size = len(self.chars) # 得到字符的索引，这点在文本处理的时候是值得借鉴的 
            print(self.vocab_size)
            self.vocab = dict(zip(self.chars, range(len(self.chars)))) # 将字符写入文件 
            with open(vocab_file, 'wb') as f: 
                pickle.dump(self.chars, f) # 使用map得到input文件中1115394个字符对应的索引 
