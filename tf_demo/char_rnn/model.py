        ...
        if not training: 
            ...
            for _ in range(args.num_layers): 
                ...
                if training and (args.output_keep_prob < 1.0 or args.input_keep_prob < 1.0): 
                    ...
                    cells.append(cell) # 通过cells列表，构建多层RNN 
            self.cell = cell = rnn.MultiRNNCell(cells, state_is_tuple=True)
            self.input_data = tf.placeholder( tf.int32, [args.batch_size, args.seq_length])
            self.targets = tf.placeholder( tf.int32, [args.batch_size, args.seq_length])
            self.initial_state = cell.zero_state(args.batch_size, tf.float32) 
            # 定义需要训练的权重和偏置，因为需要和[batch,rnn_size]大小的split片相乘， # 所以需要定义shape为[args.rnn_size, args.vocab_size] 
...
此时decoder_inputs变量除了第一个元素之外其他元素会被忽略。其形式定义为：loop(prev, i)=next。
 prev是[batch_size x output_size]，i是表明第i步，next是[batch_size x input_size]。
            ...
            tvars = tf.trainable_variables() # 梯度截断 
