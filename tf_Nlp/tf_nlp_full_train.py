...
class PTBModel(object):
    def __init__(self, is_training, batch_size, num_steps):
        ...
        # define vocab_vector matrix of vocab
        embedding = tf.get_variable("embedding", [VOCAB_SIZE, HIDDEN_SIZE])
        # Make input words into vocabulary vector
        ...
        if SHARE_EMB_AND_SOFTMAX:
            ...
            weight = tf.get_variable_scope("weight", [HIDDEN_SIZE, VOCAB_SIZE])
        bias = tf.get_variable_scope("bias", [VOCAB_SIZE])
        logits = tf.matmul(output, weight) + bias
 
        # define cross_entropy_logits function and the average loss
...
def run_epoch(session, model, batches, train_op, output_log, step):
    ...
    for x, y in batches:
        ...
        cost, state, _ = session.run(
            [model.cost, model.finial_state, train_op],
            {model.input_data:x, model.targets: y, model.initial_state:state}
        )
        total_costs += cost
        iters += model.num_steps
 
        # output log as training
